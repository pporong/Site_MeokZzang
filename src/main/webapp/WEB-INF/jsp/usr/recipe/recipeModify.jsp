...
function RecipeModify_submitForm(form) {
	...
	if (RecipeModify_submitFormDond) {
		alert('처리중...');
		return;
	}
...
<section class="writeRecipeSection con">
	<form id="form" class=" py-4 " action="../recipe/doModify" method="POST" enctype="multipart/form-data" onsubmit="RecipeModify_submitForm(this) return false;" name="do-Modify-recipe-form">
	...
	<div class="recipe_title flex justify-center">
		<div class="titleImgBox" style="width: 530px; height: 300px;">
			<div class="mt-8" style="width: 100%; height: 335px; border: 1px solid grey;">
				...
				src="${rq.getMainRecipeImgUri(recipe.recipeId)}" onerror="${rq.mainRecipeFallbackImgOnErrorHtml}" alt="요리 완성 사진" />
				
				<div class="font-bold" style="margin-top: 10px;">▶ 완성 된 요리사진을 (재)등록 해 주세요</div>
		...
		<div class="recipeInfoWrap mx-10">
			<div class="titleBox mt-8 " style="width: 680px;">
				...
				<input name="recipeName" class="w-full input input-bordered" style="width: 100%; height: 60px; padding: 20px;" type="text" placeholder="제목을 입력 해 주세요" value="${recipe.recipeName}"/>
			</div>
			...
			<div class="bodyBox mt-8">
				...
				<input class="w-full input input-bordered" style="height: 160px; padding: 20px; overflow:auto; white-space:nowrap;" name="recipeBody"
				 placeholder="자유롭게 레시피를 소개 해 주세요!" value="${recipe.recipeBody}" />
			</div>
		</div>
	</div>
	...
	<div class="recipeBox row mt-10" style="padding-bottom: 15px; margin-left: 100px; text-align: center;">
		<div class="stuffBox cell" id="stuffBox" style="width: 45%;">
			...
			<!-- 삭제 버튼 -->
			<p>
				<c:forEach var="stuff" varStatus="status" items="${recipe.recipeStuff }" >
		...
		<!-- 삭제 버튼 -->
			<p>
				<c:forEach var="sauce" varStatus="status" items="${recipe.recipeSauce }" >
	...
	<div class="cookWrap mt-8 con" style="border-top: 1px solid #304899;">\
				...
				<div class="flex h-full bg-gray-50 rounded-lg justify-center" style="margin-top: 20px; height: 400px; width: 100%;">
					<div class="flex justify-center rounded-xl my-auto text-center"> 
						<label for="input-recipeOrder__${ status.count }">
							<!-- 사진 미리보기 -->
							<img id="preview-recipeOrder__${ status.count }" class="multiple-container rounded-md" style="margin: 12px; width: 500px; height: 300px;" name="recipeBodyImg" 
							 onerror="${rq.orderFallbackImgOnErrorHtml}" src="${rq.getRecipeOrderImgUri(recipe.recipeId, status.count)}" alt="조리 과정 사진"/>
							<input type="file" id="input-recipeOrder__${ status.count }" multiple="multiple" accept="image/*"name="file__order__0__extra__recipeOrderImg__${ status.count }" class=" recipeOrderBox" />
						</label>
					</div>
					...
					<div class="recipeBodyMsgBox ml-6 " style="width: 800px;">
						<div class="flex justify-center bg-gray-50 rounded-md " style="margin-top: 36px;">
							<input name="recipeMsgBody" class="w-full h-full text-lg p-3 border border-gray-300 rounded-lg"
							 style="height: 300px;" required placeholder="조리 과정을 입력해주세요." value=" ${body}" />
						</div>
					</div>
					<button type="button" onclick="remove_orderBox(this);" class="btn btn-sm btn-outline fc_redH">삭제</button>
				</div>
			</c:forEach>
		</div>
		...
		<div class="btn_box mt-8 flex justify-center">
			<button id="modifySubmitBtn" class="btn btn-sm fc_redH" type="submit" value="수정">수정</button>
		</div>
		
	</form>

		<div class="btns my-4 flex justify-end">
			<a class="fc_redH" style="margin-right: 35px;" href="${param.replaceUri }">뒤로가기</a>
		</div>
